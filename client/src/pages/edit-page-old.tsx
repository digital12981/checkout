...
export default function EditPage() {
    ...
    return new Promise<string>((resolve) => {
      setTimeout(() => {
        try {
          ...
          let previewElement = document.getElementById('form-preview-capture');
          
          if (!previewElement) {
            // Try to find the preview container in the right panel
            const rightPanel = document.querySelector('.flex-1.bg-gray-50.overflow-auto');
            if (rightPanel) {
              previewElement = rightPanel.querySelector('[style*="background-color"]') as HTMLElement;
            }
          }
          
          if (!previewElement) {
            // Try to find any element with the background color style
            previewElement = document.querySelector('div[style*="min-h-screen"][style*="background-color"]') as HTMLElement;
          }
          
          if (!previewElement) {
            console.log("Preview element not found in any location");
  ...
  const capturePreviewHTML = () => {
    console.log("Starting HTML capture from preview...");
    ...
    setTimeout(() => {
      ...
      if (previewContent) {
        console.log("Found preview content, capturing...");
        
        // Clone and clean the content
        const cloned = previewContent.cloneNode(true) as HTMLElement;
        
        // Remove all editor-specific attributes and classes
        const removeEditorElements = (element: Element) => {
          ...
          // Remove editor classes
          const editorClasses = ['cursor-pointer', 'ring-2', 'ring-primary', 'hover:ring-2', 'transition-colors', 'z-10', 'border-dashed', 'hover:bg-gray-50'];
          ...
          // Process children
          Array.from(element.children).forEach(removeEditorElements);
        };
        
        removeEditorElements(cloned);
        
        let capturedHTML = cloned.outerHTML;
        
        // Replace the form content area with placeholder
        capturedHTML = capturedHTML.replace(
          /<div[^>]*class="[^"]*bg-white[^"]*border[^"]*border-gray-200[^"]*rounded-lg[^"]*p-6[^"]*max-w-md[^"]*mx-auto[^"]*"[^>]*>[\s\S]*?<\/div>/g,
        ...
        console.log("Contains FORM_PLACEHOLDER:", capturedHTML.includes('{{FORM_PLACEHOLDER}}'));
        
        // Store the captured HTML for saving
        (window as any).capturedPreviewHTML = capturedHTML;
        
        // Show success message
        const statusEl = document.getElementById('capture-status');
        if (statusEl) {
          statusEl.textContent = `✓ HTML capturado (${capturedHTML.length} caracteres)`;
          statusEl.className = 'text-green-600 text-sm font-medium';
        }
        
      } else {
        console.error("Preview content not found");
        document.getElementById('capture-status')!.textContent = '✗ Erro: Preview não encontrado';
        document.getElementById('capture-status')!.className = 'text-red-600 text-sm font-medium';
      }
    }, 500);
  };
  ...
  const onSubmit = async (data: EditPageForm) => {
    try {
      ...
      // Use captured HTML if available, otherwise generate from data
      let finalHTML = (window as any).capturedPreviewHTML;
      
      if (!finalHTML || finalHTML.length < 500) {
        console.log("No captured HTML, generating from data...");
        finalHTML = generateCleanHTML(data, customElements);
      } else {
        console.log("Using captured preview HTML");
      }
